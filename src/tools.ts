...
export class OrgTools extends Tools {
  ...
  @tool("Moves a task under a new parent", {
    ...
    position:
      "Position relative to siblings ('first', 'last', '1', '2', '3', ...)",
  })
  moveTask({
    id,
    newParentId,
    ...
    try {
      let pos: string | number = position;
  ...
  @status("Get contents of Org-Mode file")
  getStatus() {
    const text = readFileSync("knowledge.org", "utf8");
    return `Current time: ${
      new Date(Date.now() + this.offset * 1000).toUTCString().slice(0, -4)
